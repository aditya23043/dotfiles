...
void loop(){
  ...
  if(digitalRead(pin_sym_2) == LOW){
    i2c_data_to_send[2] = true;
  } else {
    i2c_data_to_send[2] = false;
  }
  ...
  // sym 1
  } else if(i2c_data_received[0] == true){
    for(int i = 0; i < num_pins; i++){
      current_layout[i] = sym_1[i];
    }
    current_layout_name = "sym_1";
  // sym 2
  } else if(i2c_data_to_send[2] == true && i2c_data_received[0] == true){
    for(int i = 0; i < num_pins; i++){
      current_layout[i] = sym_2[i];
    }
    current_layout_name = "sym_2";
  }
  ...
  for(int i = 0; i < num_pins; i++){
    if(digitalRead(pins[i]) == LOW){
      if(!key_down[i]){
        if((current_layout_name == "alpha_2" && alpha_1[i] != layout_alpha_2_key) || (current_layout_name == "num" && alpha_1[i] != layout_num_key) || (current_layout_name == "sym_1" && alpha_1[i] != layout_sym_1_key) || (current_layout_name == "alpha_1")){
